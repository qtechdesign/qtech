...
import com.app.frostprotectionsystemandroid.data.source.remote.network.CustomCallAdapter
import io.reactivex.*
...
open class BaseRxCallAdapterWrapper<R>(private val type: Type, private val retrofit: Retrofit, private val wrapped: CallAdapter<R, *>?) : CallAdapter<R, Any> {
    ...
    override fun adapt(call: Call<R>): Any? {
        ...
        return when (adapt) {
            ...
            is Completable -> adapt.onErrorResumeNext(this::handleCompletableRetrofitError)
    ...
    private fun handleCompletableRetrofitError(t: Throwable): CompletableSource? =
        Completable.error { convertRetrofitExceptionToCustomException(t, retrofit) }
