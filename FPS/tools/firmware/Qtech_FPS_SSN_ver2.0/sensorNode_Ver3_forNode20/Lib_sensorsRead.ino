...
#define SHT10_Air_clockPin     42
#define SHT10_Air_dataPin      43

SHT1x Air_Humidity_Sensor(SHT10_Air_dataPin, SHT10_Air_clockPin);

#define win_Direction_Pin A3
#define win_Speed_Pin A2
#define battery_Value_Pin A4
#define air_pressure_Data_pin A0
#define water_pressure_Data_pin A0
...
float get_Soil_Moisture_Data()
{
  ...
  {
    analogData += analogRead(soil_Moisture_Pin);
  }
  analogData = analogData/timesReadSensor;
  int soilMoistureData = (100-map(analogData, 0, 1023, 0, 100));
...
{
  float humidity;
  humidity = Air_Humidity_Sensor.readHumidity();
  if (humidity > 100)
  {
    humidity = 100;
  }
