...
UDOpenLora::UDOpenLora(HardwareSerial* LoraSerial)
{
...
{
  char mess[256];
  int lengthOfMess = sprintf (mess, "%c%c%c\"L_DT\":\"%s\"", (byte)(0xA0), (byte)(0xA1), (byte)(0xA2), nameOfDevice);
...
RET_STATUS UDOpenLora::ReceiveMsg(byte *ADDRH, byte *ADDRL, byte *pdatabuf, uint8_t *data_len)
{
  ...
  {
    delay(100);
    if(this->LoraSerial->available() >= 2)
    {
